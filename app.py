...
# --- 3. THE FIXED ID CARD GENERATOR ---

def create_id_card(s_info, photo_url=None):
    """Generates a CR80 ID Card. Uses robust download logic for photos."""
    ...
    if photo_url:
        try:
            # Force a stream download to ensure we get the full image data
            response = requests.get(photo_url, stream=True, timeout=10)
            if response.status_code == 200:
                img_data = BytesIO(response.content)
                # Position: x=17, y=25, w=20, h=25
    ...
    if not image_loaded:
        # Fallback box if the photo fails to load
        pdf.set_fill_color(240, 240, 240)
        pdf.rect(17, 25, 20, 25, 'F')
        pdf.set_text_color(150, 150, 150)
        pdf.set_font("Arial", '', 6)
        pdf.text(21, 38, "NO PHOTO")
        
    # Student Details
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", 'B', 9)
    pdf.text(5, 58, str(s_info['name']).upper())
...
if not st.session_state.auth["logged_in"]:
    ...
    if st.session_state.auth["role"] == "admin":
        ...
        with t3:
            ...
            if not df.empty:
                target_id = st.selectbox("Select Student ID for Photo", df['id'].tolist())
                ...
                if st.button("Update Profile Photo") and p_file:
                    ...
                    # Upsert handles overwriting the old photo
                    ...
                    with st.expander(f"👤 {row['id']} | {row['name']} | Password: {row['pass']}"):
                        ...
                        with c_b:
                            try:
                                ...
                                st.info("Attendance tracking active.")
    ...
    # --- STUDENT SIDE ---
        ...
        with cl1:
            ...
            if st.button("🪪 Download My ID"):
                # Fetch fresh data to ensure we have the photo URL
                res = supabase.table("students").select("profile_photo_url").eq("id", u['id']).execute()
                p_url = res.data[0]['profile_photo_url'] if (res.data and 'profile_photo_url' in res.data[0]) else None
                
                # Check if it's a valid string
                if p_url and p_url.strip():
                    pdf_bytes = create_id_card(u, p_url)
                else:
                    pdf_bytes = create_id_card(u, None)
